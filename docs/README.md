...
1. **廣告**：在 `config.js` 填入廣告 slot ID，並把廣告平台提供的 SDK 初始化集中放在 `app.js`。頂部使用展示廣告；`reward-button` 的 click handler 是獎勵式廣告完成後發放獎勵的唯一入口。請遵守各平台對遊戲、年齡及同意管理的政策。
