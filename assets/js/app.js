(() => {
  ...
  function startGame() { if (running) return; resetGame(); running = true; message.hidden = true; message.style.display = "none"; start.disabled = true; start.textContent = "挑戰中…"; deadline = performance.now() + cfg.game.durationSeconds * 1000; spawnOrb(); spawnTimer = setInterval(spawnOrb, cfg.game.spawnIntervalMs); tick(); track("game_start"); }
  ...
  $("close-ad").addEventListener("click", () => { $("floating-ad").hidden = true; track("floating_ad_closed"); });
